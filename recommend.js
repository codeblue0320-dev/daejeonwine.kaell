...
const STUDY_RAW=JSON.parse(fs.readFileSync(path.join(DIR,'varieties.json'),'utf8'));
// 앱과 동일한 셔플(seed 20260710, python random.Random 결과를 앱 빌드시 굽지만, 여기선 순서 파일이 이미 셔플본이 아님)
// varieties.json은 원본 순서이므로 앱과 동일한 순서를 재현하기 위해 동일 셔플 결과 인덱스를 사용:
const ORDER=JSON.parse(fs.readFileSync(path.join(DIR,'study_order.json'),'utf8'));
const STUDY=ORDER.map(i=>STUDY_RAW[i]);
...
// 품종 공부 스케줄 (앱과 동일)
function monthCal(mi){const m0=7+mi;return{y:2026+Math.floor((m0-1)/12),m:((m0-1)%12)+1};}
function dim(y,m){return new Date(y,m,0).getDate();}
function learnDays(mi){const c=monthCal(mi);return mi===0?dim(c.y,c.m)-9:dim(c.y,c.m);}
function learnOffset(mi){let off=0;for(let k=0;k<mi;k+=2)off+=learnDays(k);return off;}
function studyFor(ds){
  const p=ds.split('-').map(Number);const y=p[0],m=p[1],d=p[2];
  const mi=(y-2026)*12+(m-7);if(mi<0||(mi===0&&d<10))return null;
  const N=STUDY.length;
  if(mi%2===0){const dayIdx=(mi===0)?d-10:d-1;return{mode:'learn',v:STUDY[(learnOffset(mi)+dayIdx)%N],num:dayIdx+1,total:learnDays(mi)};}
  const off=learnOffset(mi-1),len=learnDays(mi-1);
  if(d<=len)return{mode:'review',v:STUDY[(off+d-1)%N],num:d,total:len};
  return{mode:'bonus',v:STUDY[(off+((d-1)%len))%N],num:len,total:len};
}
...
console.log(JSON.stringify({
  ...
  study:st?{mode:st.mode==='learn'?'새 품종 학습':st.mode==='review'?'복습':'자유 복습',ko:st.v.ko,en:st.v.en,num:st.num,total:st.total}:null
},null,1));
